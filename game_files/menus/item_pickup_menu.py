...
def choose_item_to_pick_up(encountered_items_list: list, player: Player) -> None:
    items_to_add = []
    user_choice = input(
    ...
    if user_choice == "2":
        while True:
            ...
            for current_index in item_choice_menu_indexes:
                ...
                hand_multiple_items_to_inventory(items_to_add, player)
            return
    if user_choice == "1":
        ...
        hand_one_item_to_inventory(item_to_add, player)
    else:
        print("No such option. Try again, dumbass")
        return


def hand_one_item_to_inventory(item_to_add: Item, player: Player) -> None:
    player.inventory.add_item(item_to_add)
    print(f"{item_to_add} successfully added to inventory!")


def hand_multiple_items_to_inventory(items_to_add: list, player: Player) -> None:
