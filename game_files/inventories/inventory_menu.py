...
from game_files.inventories import inventory_slot_mediator
from game_files.items.usable_item import UsableItem


def select_item(player_character: Player) -> None:
...
def move_equipped_item_back_to_inventory(player_character: Player) -> None:
...
def inventory_navigation(player_character: Player) -> None:
    while True:
        ...
        if user_input not in {"1", "2", "3"}:
            ...
            move_equipped_item_back_to_inventory(player_character)
        else:
            select_item(player_character)
