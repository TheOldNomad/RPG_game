...
def pick_new_skill(cleric_character: ClericPlayer) -> None:
    cleric_character.skill_tree.view_relevant_skill_tree()
    skill_to_pick = int(input("Please, pick the skill you like"))
    current_skill = cleric_character.skill_tree.choose_skill(skill_to_pick)
    ...
    if player_command == "1":
        ...
        return
    if player_command == "2":
        cleric_character.skill_tree.acquire_skill(cleric_character, skill_to_pick)
    else:
        return
